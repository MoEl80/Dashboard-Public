(function(){
  ...
  window.showPair=function(k){
    ...
    try{
      ...
      const html=`<div class="section wide"><h3>Technical Engine v2.1 — indicators + market structure</h3><div class="muted" style="margin-bottom:12px">Each H1/H4/D/W timeframe independently combines correlation-adjusted indicator families (70%), 8-currency relative strength (20%), and correlation-confirmed cross-pair evidence (10%). Only after that are timeframe weights H1 15% / H4 30% / Daily 35% / Weekly 20% applied. Phase 1 remains preserved for audit.</div><div class="tf-grid"><div class="tf"><h4>Ensemble summary</h4><div class="kv"><div>Final technical score</div><div class="${scoreClass(t.technical_score)}">${num(t.technical_score)}</div><div>Final confidence</div><div>${num(t.technical_confidence)}%</div><div>Pre-exposure confidence</div><div>${num(t.technical_confidence_pre_exposure)}%</div><div>Weighted indicator quality</div><div>${num(v.weighted_indicator_confidence)}%</div><div>Weighted structure quality</div><div>${num(v.weighted_structure_confidence)}%</div><div>Weighted exposure crowding</div><div>${num(v.weighted_exposure_crowding)}%</div><div>Total instances</div><div>${esc(v.instances_succeeded??'—')} / ${esc(v.instances_attempted??'—')}</div></div></div><div class="tf"><h4>Legacy Phase 1 comparison</h4><div class="kv"><div>Legacy score</div><div class="${scoreClass(legacy.technical_score)}">${num(legacy.technical_score)}</div><div>Legacy confidence</div><div>${num(legacy.technical_confidence)}%</div><div>Legacy alignment</div><div>${num(legacy.alignment_score)}%</div><div>Preserved</div><div>${t.phase1_preserved?'Yes':'—'}</div></div></div></div><div class="tf-grid" style="margin-top:12px">${['H1','H4','D','W'].map(name=>timeframeCard(name,tf[name]||{})).join('')}</div></div>`;
